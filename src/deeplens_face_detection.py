    ...
    try:
        ...
        while doInfer:
            ...
            for obj in parsed_results:
                ...
                #Adjust the cropping to be more zoomed out
                xmin = max(0, int( xscale * obj['xmin'] ) + int((obj['xmin'] - input_width/2) + input_width/2) - 100)
                ymin = max(0, int( yscale * obj['ymin'] ) - 100)
                xmax = min(int( xscale * obj['xmax'] ) + int((obj['xmax'] - input_width/2) + input_width/2) + 100, frame.shape[1])
                ymax = min(int( yscale * obj['ymax'] ) + 100, frame.shape[0])
